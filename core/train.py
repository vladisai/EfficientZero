...
def load_train_state(exp_path):
    with open(os.path.join(exp_path, "train_state.yaml"), "r") as f:
...
def update_weights(
    model: BaseNet, batch, optimizer, replay_buffer, config, scaler, vis_result=False
):
    ...
    # In our modified EfficientZero repo, we apply consistency (SSL) loss on tensors of all timesteps [batch, time, ...]
    consist_preds = []
    consist_targs = []
    ...
    with contextlib.ExitStack() as stack:
        ...
        for step_i in range(config.num_unroll_steps):
            ...
            if config.consistency_coeff > 0:
                consist_preds.append(
                    model.project(hidden_state),
                )
                # obtain the oracle hidden states from representation function
                with torch.no_grad():
                    consist_targs.append(
                        model.project(
...
def _train(
    ...
    while not (
        ...
        time.sleep(1)
        print(
            f"Waiting for replay buffer: {ray.get(replay_buffer.get_total_len.remote())} < {config.start_transitions}"
        )
        pass
...
def train(config, summary_writer, model_path=None, buffer_path=None):
    """training process
    Parameters
    ----------
    summary_writer: Any
    ...
    if model_path:
        ...
        train_state = load_train_state(config.exp_path)
